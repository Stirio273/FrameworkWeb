...
public class Util {
    ...
    public static void setMappingUrls(HashMap<String, Mapping> urls, String path, List<String> packages) throws Exception {
        for (int i = 0; i < packages.size(); i++) {
            List<Class<?>> modeles = getTypesAnnotedWith(packages.get(i), RequestMapping.class, path);
            ...
            for (int n = 0; n < modeles.size(); n++) {
                ...
                for (Method methode : methodes) {
                    key = modeles.get(n).getAnnotation(RequestMapping.class).path()
                            + methode.getAnnotation(RequestMapping.class).path();
    ...
    public static List<Class<?>> getTypesAnnotedWith(String nomPackage, Class<? extends Annotation> annotation, String path) {
        try {
